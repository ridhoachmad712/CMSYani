...
@if ($members->isNotEmpty())
    <section id="tim-kami" class="border-t border-navy/5 bg-white py-14 sm:py-20 lg:py-24">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="reveal flex max-w-2xl flex-col mx-auto text-center items-center">
                <span class="mb-2 text-sm font-semibold uppercase tracking-widest text-gold-dark">Profesional &amp; Berpengalaman</span>
                <h2 class="text-3xl font-bold sm:text-4xl text-navy">Tim Konsultan &amp; Rekan</h2>
                <div class="mt-3 h-1 w-20 rounded bg-gold mx-auto"></div>
            ...
            <div class="reveal mt-14 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($members as $member)
                    ...
                    @endphp
                    <div class="group relative flex flex-col overflow-hidden rounded-2xl border border-navy/10 bg-cream/50 p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:border-gold/50 hover:bg-white hover:shadow-xl">
                        {{-- Foto Profil --}}
                        ...
                        <h3 class="text-xl font-bold text-navy group-hover:text-gold-dark transition duration-200">
                            {{ $member->name }}
                        </h3>
                        ...
                        @if ($member->email || $member->linkedin_url)
                            <div class="mt-6 flex items-center gap-3 border-t border-navy/10 pt-4 text-xs font-medium text-navy/60">
                                ...
                                @if ($member->linkedin_url)
                                    <a href="{{ $member->linkedin_url }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 transition hover:text-gold-dark ml-auto" title="Profil LinkedIn">
